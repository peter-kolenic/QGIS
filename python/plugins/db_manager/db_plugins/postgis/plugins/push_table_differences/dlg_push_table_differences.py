...
from .ui_DlgPushTableDifferences import Ui_DbManagerDlgPushTableDifferences as Ui_Dialog
from .ui_DlgPushTableDifferences import _fromUtf8
...
class DlgPushTableDifferences(QDialog, Ui_Dialog):
	...
	def syncFinished(self, success, inserts, updates, deletes, has_privileges):	# has_privileges is ignored here
		...
		self.printMessage("")
		self.printMessage(self.tr("%s while pushing differences: inserts :%d  updates: %d  deletes: %d") %
...
class DBScanForPushCompatibleTables(QObject):
	...
	def __init__(self, input_table_ref, tr, databaseConnection = None):
		"If databaseConnection argument is set, search only in given database"
		QObject.__init__(self)
...
class Table(object):
	...
	def pg_comparator_connect_string(self, force_pk = None):
		...
		s = 'pgsql://%(username)s%(pass)s@%(host)s%(port)s/%(base)s/%(schema)s"%(table)s"?%(pk)s' % {
			"username":username,
			"pass": ':%s' % password if  password else '',
			"host":	host,
